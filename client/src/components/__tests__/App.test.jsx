...
describe('App', () => {
  const mockCities = [
    { code: 'vilnius', name: 'Vilnius', administrativeDivision: 'Vilniaus m. sav.' },
  ...
  it('limits most viewed cities to 3', async () => {
    const savedCities = [
      { code: 'vilnius', name: 'Vilnius', administrativeDivision: 'Vilniaus m. sav.' },
      { code: 'kaunas', name: 'Kaunas', administrativeDivision: 'Kauno m. sav.' },
      { code: 'klaipeda', name: 'Klaipėda', administrativeDivision: 'Klaipėdos m. sav.' }
    ];
    localStorage.setItem('mostViewedCities', JSON.stringify(savedCities));
    ...
    // Simulate adding a new city by directly updating localStorage
    const newCity = { code: 'palanga', name: 'Palanga', administrativeDivision: 'Palangos m. sav.' };
    
    // Get current cities
    const currentCities = JSON.parse(localStorage.getItem('mostViewedCities') || '[]');
    
    // Add new city (simulating what App component would do)
    const updatedCities = [
      newCity,
    ...
    // Verify localStorage still has only 3 cities
    const storedCities = JSON.parse(localStorage.getItem('mostViewedCities'));
    expect(storedCities.length).toBeLessThanOrEqual(3);
    
    // New city should be first
    expect(storedCities[0].name).toBe('Palanga');
    
    // Oldest city (Klaipėda) should be removed
